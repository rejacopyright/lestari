...
@section('title') {{strtoupper($series->product->brand->name)}} {{strtoupper($series->product->name)}} {{strtoupper($series->name)}} @endsection
@section('css')
<meta http-equiv="Content-Language" content="id" />
<meta property="og:title" content="{{strtoupper($series->name)}}"/>
<meta property="og:description" content="Brand : {{strtoupper($series->product->brand->name ?? 'OTHER')}}"/>
<meta name="description" content="Brand : {{strtoupper($series->product->brand->name ?? 'OTHER')}}"/>
<meta property="og:type" content="image/jpeg"/>
<meta property="og:url" content="{{url()->full()}}"/>
<meta property="og:site_name" content="{{url('/')}}"/>
<meta property="og:image" itemprop="image" content="{{url('public')}}/images/series/{{$series->image ?? '../add-image.png'}}?{{time()}}"/>
<meta property="og:image:secure_url" content="{{url('public')}}/images/series/{{$series->image ?? '../add-image.png'}}?{{time()}}"/>
  ...
  <div class="row">
    <div class="col-lg-3">
      <div class="d-flex align-items-center hpx-200">
        <img src="{{url('public')}}/images/series/{{$series->image ?? '../image.png'}}?{{time()}}" class="h-100 p-2 mx-auto">
      </div>
      <h5 class="text-center m-0 bg-gray py-2 bold text-uppercase">{{$series->name}}</h5>
      <div class="mt-2">
        <a href="{{url('product/spec')}}?product_id={{$series->product->product_id}}&series_id={{$series->series_id}}" class="btn btn-block btn-info-light radius-5 mt-1"><i class=""></i>Find Specification</a>
      </div>
      <div class="mt-2">
        <a href="{{url('product/detail')}}/{{$series->product->product_id}}" class="btn btn-block btn-light radius-5 mt-1"><i class="icon-line-awesome-undo mr-1"></i>Back</a>
      </div>
    </div>
    <div class="col-lg-9">
      <div class="tabs">
              ...
              <div id="series-list">
                <div class="row">
                  <?php foreach ($model as $mdl): ?>
                    <?php
                     $href = url('product/spec').'?product_id='.$series->product->product_id.'&series_id='.$mdl->series_id.'&model_id='.$mdl->model_id;
                     ?>
                    <div class="col-md-2 col-sm-4 col-6 text-center mb-3">
                      <div class="card p-2 pointer shadow-xs" data-tippy-placement="top" title="{{$mdl->name}}" onclick="window.location.href = '{{$href}}'">
                        <img src="{{url('public')}}/images/series/{{$mdl->image ?? '../image.png'}}?{{time()}}" class="w-75">
                        <p class="text-dark my-2 text-uppercase font-10 bold text-truncate">{{$mdl->name}}</p>
                      </div>
                    </div>
                  <?php endforeach; ?>
                </div>
