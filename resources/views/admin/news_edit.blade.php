...
@endsection
@section('content')
<form class="" action="{{url('admin/news/put')}}" method="post" enctype="multipart/form-data">
  @csrf
  ...
  <div class="px-4 pt-3">
    <div class="row">
      <div class="col-auto text-center">
        <div class="dashboard-box m-0 p-3">
          <div class="avatar-wrapper d-inline-block" data-tippy-placement="bottom" title="Upload Image">
            <img class="profile-pic" src="{{url('public')}}/images/news/{{$news->image ?? '../add-image.png'}}?{{time()}}" alt="" />
      ...
      <div class="col">
        <div class="dashboard-box m-0 p-3">
          <div class="mb-2">
            <input type="text" class="flat" name="title" value="{{$news->title}}" placeholder="News Title" required>
          </div>
          <div class="mb-2">
            <p class="m-0 text-muted">News Description</p>
            <textarea name="description" class="flat summernote" placeholder="News Description" required>{{$news->description}}</textarea>
          </div>
